...
def get_llm_travel_insight(city, vibes, interests, activity_level, match_pct):
    ...
    containing a personalized narrative, 3 activity suggestions, and a packing tip.
    The JSON output is then parsed and used to drive the UI — making this
    non-straightforward: LLM output feeds structured display, not just raw text.
    """
    if not co:
        return None
...
Remember: respond ONLY with the JSON object, nothing else."""
    ...
    try:
        ...
        # Second attempt with stricter prompt if first fails
...
def get_chatbot_response(user_message, chat_history, city, vibes, interests):
    """
    Multi-call chatbot: sends full conversation history on every call.
    System prompt is personalized with the recommended city and user profile.
    This is non-straightforward: multi-turn conversation with stateful history.
    """
    if not co:
...
.chat-header {
    ...
    font-weight: 700; color: #1a1a2e; margin-bottom: 4px;
}
...
<div class="hero">
    <h1>Where to next?</h1>
    <p>Tell us your vibe — we'll find your perfect European escape, with AI-powered insights and live events.</p>
</div>
""", unsafe_allow_html=True)
...
if clicked:
    ...
    # Reset chat when new recommendation is made
    st.session_state.chat_history = []
...
# ── DISPLAY RESULTS (persists across chat reruns) ──────────────────────────────
